<?php
    ...
    require('process/t-list-analysedefaillance.php');
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">
                            Liste des analyses défaillances
                        </h1>
                        <ol class="breadcrumb">
                            <li class="active">
                                <a href="\choiqualitycontrol\admin\index.php"><i class="fa fa-dashboard"></i> Accueil</a> > <small>Liste des expertises d'analyse</small>
                            </li>
                        </ol>
                    </div>
                </div>
                            ...
                            <tbody>
                                <?php while ($prest = $prestations->fetch()) { ?>
                                    <tr>                                        
                                        ...
                                        <td class="text-center">
                                            <span>
                                                <a role="button" title="Modifier la prestation" href="\choiqualitycontrol\admin\edit-analysedefaillance.php?nom=<?= $prest['service_nom'];?>&id=<?= $prest['service_id']?>"
                                                   class="edit fa fa-pencil" id="edit-<?= $prest['service_id']?>"></a>
                                            </span>

                                            <span>
                                                <?php if ($prest['service_active'] == 1) {?>
                                                <a role="button" title="Désactier la prestation" href="\choiqualitycontrol\admin\list-analysedefaillance.php?act=disable&nom=<?= $prest['service_nom'];?>&id=<?= $prest['service_id']?>"
                                                   class="del fa fa-fw fa-trash" id="del-<?= $prest['service_id']?>"></a>
                                                <?php }else {?>
                                                <a role="button" title="Actier la prestation" href="\choiqualitycontrol\admin\list-analysedefaillance.php?act=enable&nom=<?= $prest['service_nom'];?>&id=<?= $prest['service_id']?>"
                                                   class="del fa fa-fw fa-level-up" id="del-<?= $prest['service_id']?>\choiqualitycontrol"></a>
                                                <?php }?>
                                            </span>
