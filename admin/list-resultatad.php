<?php
    ...
    require('process/t-list-resultatad.php');
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">
                            Liste des resultats des analyse défaillances
                        </h1>
                        <ol class="breadcrumb">
                            <li class="active">
                                <a href="\choiqualitycontrol\admin\index.php"><i class="fa fa-dashboard"></i> Accueil</a> > <small>Liste des résultats des analyses défaillances</small>
                            </li>
                        </ol>
                    </div>
                </div>
                            ...
                            <tbody>
                                <?php while ($prest = $prestations->fetch()) { ?>
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <img src="/media/img/prest/<?= $prest['exemple_photo'];?>"
                                                 style="width: 60px; height: 60px"/>
                                        </td>
                                        <td class="text-center">
                                            <span>
                                                <a role="button" title="Modifier le résultat" href="\choiqualitycontrol\admin\edit-resultatad.php?nom=<?= $prest['exemple_idnom'];?>&id=<?= $prest['exemple_id']?>"
                                                   class="edit fa fa-pencil" id="edit-<?= $prest['exemple_id']?>"></a>
                                            </span>

                                            <span>
                                                <?php if ($prest['exemple_active'] == 1) {?>
                                                <a role="button" title="Désactier le résultat" href="\choiqualitycontrol\admin\list-resultatad.php?act=disable&nom=<?= $prest['exemple_idnom'];?>&id=<?= $prest['exemple_id']?>"
                                                   class="del fa fa-fw fa-trash" id="del-<?= $prest['exemple_id']?>"></a>
                                                <?php }else {?>
                                                <a role="button" title="Actier le résultat" href="\choiqualitycontrol\admin\list-resultatad.php?act=enable&nom=<?= $prest['exemple_idnom'];?>&id=<?= $prest['exemple_id']?>"
