<?php 
	...
	// Récupération des qualités en bd
	$type = $db->prepare('SELECT * FROM qualite WHERE qualite_active=1');
	$type->execute();
	// Récupération des normes en bd
	$cont = $db->prepare('SELECT * FROM norme  WHERE norme_active =1');
	$cont->execute();
	...
	if(isset($_POST['ajouter'])){
		...
		if(!empty($nom)){
			$insertPrest = $db->prepare('INSERT INTO norme(norme_nom,norme_name, norme_active,norme_date) VALUES(?,?,?,NOW())');
