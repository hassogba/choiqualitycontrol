<?php 
    ...
    if (!empty(isset($_GET['nom'])) && !empty(isset($_GET['id'])) && !empty(isset($_GET['act']))){
        ...
        if ($action == 'disable'){
            $delPrest = $db->prepare('UPDATE user SET admin = 0 WHERE user_nom = ? AND user_id = ?');
            $delPrest->execute(array($nom, $id));
        }elseif ($action == 'enable'){
            $delPrest = $db->prepare('UPDATE user SET admin = 1 WHERE user_nom = ? AND user_id = ?');
            $delPrest->execute(array($nom, $id));
        }
    ...
    $prestations = $db->prepare('SELECT * FROM user ORDER BY admin DESC');
    $prestations->execute();
