<?php 
    ...
    if (!empty(isset($_GET['nom'])) && !empty(isset($_GET['id'])) && !empty(isset($_GET['act']))){
        ...
        header('Location: ./list-contexteea.php');
        echo "Success";
    }else{
        echo "Failed";
    }
    ...
    $prestations = $db->prepare('SELECT * FROM contexte AS p, service AS c WHERE c.service_typeid=1 AND p.contexte_serviceid = c.service_id ORDER BY p.contexte_active DESC');
    $prestations->execute();
