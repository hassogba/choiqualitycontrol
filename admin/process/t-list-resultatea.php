<?php 
    ...
    if (!empty(isset($_GET['nom'])) && !empty(isset($_GET['id'])) && !empty(isset($_GET['act']))){
        ...
        header('Location: ./list-resultatea.php');
        echo "Success";
    }else{
        echo "Failed";
    }
    ...
    $prestations = $db->prepare('SELECT * FROM exemple AS p, service AS c WHERE c.service_typeid=1 AND p.exemple_serviceid = c.service_id ORDER BY p.exemple_active DESC');
    $prestations->execute();
 ?>