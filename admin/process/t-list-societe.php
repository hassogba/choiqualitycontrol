<?php 
	require('config.php');

    // Supprime des pages une prestation, celle-ci est toujours en BD.
    // En effet, la suppression ici consiste à changer l'attribut 'active' de 1 à 0
    // On récupère d'abord les variables passées passées dans l'url puis en exécute la requete
    if (!empty(isset($_GET['nom'])) && !empty(isset($_GET['id'])) && !empty(isset($_GET['act']))){
        $nom = htmlspecialchars($_GET['nom']);
        $id = htmlspecialchars($_GET['id']);
        $action = htmlspecialchars($_GET['act']);

        if ($action == 'disable'){
            $delPrest = $db->prepare('UPDATE societe SET societe_active = 0 WHERE societe_nom = ? AND societe_id = ?');
            $delPrest->execute(array($nom, $id));
        }elseif ($action == 'enable'){
            $delPrest = $db->prepare('UPDATE societe SET societe_active = 1 WHERE societe_nom = ? AND societe_id = ?');
            $delPrest->execute(array($nom, $id));
        }
        header('Location: ./list-societe.php');
        echo "Success";
    }else{
        echo "Failed";
    }

   
     // Récupère toutes les sociétés en BD
    $prestations = $db->prepare('SELECT * FROM societe ORDER BY societe_active DESC');
    $prestations->execute();
 ?>